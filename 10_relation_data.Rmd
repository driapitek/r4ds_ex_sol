...
flights %>%
  ...
  ggplot(aes(year_of_manufactered, mean_arr_delay)) +
  geom_point() +
  geom_smooth()
```
...
flights %>%
  ...
  left_join(year_planes, by = "tailnum", na.rm = TRUE, suffix = c("_of_plane", "_of_manufactered")) %>%
  group_by(tailnum, year_of_manufactered) %>%
  summarise(dep = mean(dep_delay, na.rm = TRUE),
            arr = mean(arr_delay, na.rm = TRUE)) %>%
  gather(dep, arr, key = "delay", value = "value") %>%
...
flights %>%
  select(year:day, dep_delay, arr_delay, tailnum) %>%
  left_join(year_planes, by = "tailnum", na.rm = TRUE, suffix = c("_of_plane", "_of_manufactered")) %>%
  mutate(age = year_of_plane - year_of_manufactered) %>%
  filter(!is.na(age)) %>%
