...
* Хозяйке на заметку --- образование класторв близких значений, указывает на существование подгрупп среди данных.
...
Решение с панелями я подсмотрел [Jeffrey Arnold](https://github.com/jrnold). Но конечено же я не хочу бездумно копировать поэтому прежде чем приводить его решение, я разберу новые неизвестные используемые у Джефри функции.

* Функция `gather()` --- она используется, когда нужно несколько столбцов поместить в одну переменную. Такое иногда необходимо на практике, и я сам пару раз использовал подобный приём в Excel. В дополнении PLEX Еxcell для подобных нужд даже есть специальная функция. Продемонстрирую как она работает:
