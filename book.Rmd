...
flights %>%
  ...
  ggplot(aes(month, dest)) +                          # Строим диаграмму распределения с заливкой по среднему значению
...
Её суть заключается в том, чтобы отображать на графике только те направления, по которым летали в каждый месяц года
...
Можно сортировать не по алфавиту направления, а по какому-нибудь значимому параметру, например по среднему количеству полётов в году, или по средней задержки. Для того чтобы отсортировать переменную `dest` необходимо вос
