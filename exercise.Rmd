...
Функции имеют разные статистики по умолчанию. `geom_bar()` иметт `stat_count()`, а `geom_col()` имет `stat_identity()` Т.е. `geom_bar()` считает, сколько наблюдений попадает в просматриваемый диапазон и столбец показывает количество попавших наблюдений в диапазон. А `geom_col()` - строит столбцы по количеству наблюдений.
