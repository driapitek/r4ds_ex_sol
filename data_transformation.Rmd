...
1. Получение сводных данных (суммарных), касающихся расстрояний `distance`, среднего времени задержки `mean(arr_delay)` и количества авиарейсов `count()`.
...
Есть такая штука как канал `%>%`. Канал фактически позволяет говорить "затем". Сначала сгруппируй %>% (затем) просуммируй %>% (затем) отфильтруй. Все те же самые блоки, только без промежуточных присваиваний имён.
...
Я предполагал, что параметр `na.rm` отвечает за нормальное распределение. Но это было неверное предположение. 
...
Такой график показываеn распределение, почти как гистограммы, только  вместо столбцов тут линии. Как и в гистограммах, такие графики показывают количество наблюдений, попавших в заданный диапазон. Постройнный график имеет диапазон 10. Поиграемся немного:
...
Присвой значению `delays` данные `not_canceled`. Затем сгруппируй эти данные по номерам самолётов. Затем в группах посчитаем среднее значение по параметру `arr_delay`, не учитывая пропущенные значения. Со счётчиком количества наблюдений. Затем построим точечную диаграмму рассеяния количества наблюдений от времени задержки. Точки сделаем прозрачными.

На этой диаграмме видно, что хвост у основного распределения длинный. И часто от такого хвоста необходимо будет избавляться. Помогает в этом использование пакета `ggplot` в потоке `dplyr`.
...
Дословно написанный выше код можно перевести так.
Возьми данные `delays`. Затем отфильтруй группы, в которых количество наблюдений меньше `25`. Построй точечную диаграмму `n` от `delay`. Точки сделай прозрачными.
...
Разберём написанный выше код. Сначала из большого набора данных `Lahman` возьми фрейм данных `Batting`. Присвой переменной `batters` значение `batting`. Затем группируй данные по ID игрока. Затем посчитай средний коэффициент результативности - суммарное количество ударов, делённое на суммарное количество возможностей сделать удар?. Определи ещё и суммарное количество возможностей выполнить удар `ab`. 
Возьми данные `batters`, возьми данные с количеством возможностей на удар больше 100. Построй диаграмму рассеяния возможностей удара от от коэффициента результативности. На этом же графике построй сглаженную прямую без построения доверительного интервала.
